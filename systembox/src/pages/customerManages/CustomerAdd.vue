...
export default {
  data() {
    return {
      ...
      createCustomerListUrl: this.$store.state.url + "backend/customer/create",
      rules: {
        userName: [
          {
            pattern: /^[a-zA-Z0-9]{1,16}$/,
            required: true,
            message: "请输入正确的客户名：只能包含英文和数字",
            trigger: "blur",
          },
        ],
        nickName: [
    ...
    addData(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$axios.post(this.createCustomerListUrl, pamars).then((res) => {
